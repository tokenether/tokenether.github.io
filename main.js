...
function TokenEther() {
  ...
  this.contractTokenEther = undefined;
  ...
  this.web3 = undefined;
  this.startTokenEther();
}
TokenEther.prototype.ejs = function ejs(url, element, data) {
  if ($(`#${element}`).length) {
...
TokenEther.prototype.alertInfo = function alertInfo(message) {
  console.log(message);
...
TokenEther.prototype.alertDialog = function alertDialog(message) {
  console.log(message);
...
TokenEther.prototype.alertWarning = function alertWarning(message) {
  console.log(message);
...
TokenEther.prototype.alertError = function alertError(message) {
  console.log(message);
...
TokenEther.prototype.alertSuccess = function alertSuccess(message) {
  console.log(message);
...
TokenEther.prototype.txError = function txError(err) {
...
TokenEther.prototype.alertTxResult = function alertTxResult(err, txsIn) {
...
TokenEther.prototype.enableTooltipsAndPopovers = function enableTooltipsAndPopovers() {
  $('[data-toggle="popover"]').popover({ trigger: 'hover' });
  $('[data-toggle="tooltip"]').tooltip();
};
TokenEther.prototype.logout = function logout() {
...
TokenEther.prototype.createAccount = function createAccount() {
...
TokenEther.prototype.deleteAccount = function deleteAccount() {
...
TokenEther.prototype.selectAccount = function selectAccount(i) {
...
TokenEther.prototype.addAccount = function addAccount(newAddr, newPk) {
  let addr = newAddr;
  let pk = newPk;
...
TokenEther.prototype.showPrivateKey = function showPrivateKey() {
...
TokenEther.prototype.addressLink = function addressLink(address) {
  return `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${address}`;
};
TokenEther.prototype.contractAddr = function contractAddr(addr) {
  this.config.contractTokenEtherAddr = addr;
  this.displayConnectionDescription(() => {});
  this.loading(() => {});
  this.refresh(() => {}, true, true);
};
TokenEther.prototype.displayAccounts = function displayAccounts(callback) {
...
TokenEther.prototype.displayLanguages = function displayLanguages(callback) {
...
TokenEther.prototype.selectLanguage = function selectLanguage(newLanguage) {
...
TokenEther.prototype.loadEvents = function loadEvents(callback) {
  let lastBlock = 0;
  Object.keys(this.eventsCache).forEach((id) => {
    const event = this.eventsCache[id];
    if (event.blockNumber > lastBlock && event.address === this.config.contractTokenEtherAddr) {
...
TokenEther.prototype.displayMyTransactions =
function displayMyTransactions(ordersIn, blockNumber, callback) {
  ...
  orders = orders.filter(order => order.order.contractAddr === this.config.contractTokenEtherAddr);
  ...
  events.forEach((event) => {
    try {
      if (
        event.event === 'Trade' &&
        event.address === this.config.contractTokenEtherAddr &&
...
TokenEther.prototype.displayVolumes = function displayVolumes(orders, blockNumber, callback) {
  let tokenVolumes = {};
  let pairVolumes = {};
  ...
  const events = Object.values(this.eventsCache);
  events.forEach((event) => {
    if (event.event === 'Trade' && event.address === this.config.contractTokenEtherAddr) {
  ...
  Object.keys(pairVolumes).forEach((pair) => {
    ...
    ordersFiltered = ordersFiltered.filter(
      order => order.order.contractAddr === this.config.contractTokenEtherAddr);
    // final order filtering and sorting
...
TokenEther.prototype.displayTradesAndChart = function displayTradesAndChart(callback) {
  // get the trade list
  const events = Object.values(this.eventsCache);
  const trades = [];
  events.forEach((event) => {
...
TokenEther.prototype.candlestickChart =
...
TokenEther.prototype.depthChart =
...
TokenEther.prototype.lineChart =
...
TokenEther.prototype.getOrders = function getOrders(callback) {
...
TokenEther.prototype.getOrdersByPair = function getOrdersByPair(tokenA, tokenB, callback) {
...
TokenEther.prototype.getTopOrders = function getTopOrders(callback) {
...
TokenEther.prototype.displayOrderbook = function displayOrderbook(ordersIn, blockNumber, callback) {
  // only look at orders for the selected token and base
  let orders = ordersIn.filter(
    x =>
      (x.order.tokenGet.toLowerCase() === this.selectedToken.addr.toLowerCase() &&
...
TokenEther.prototype.displayTokensAndBases = function displayTokensAndBases(callback) {
...
TokenEther.prototype.displayAllBalances = function displayAllBalances(callback) {
  ...
  async.map(
    ...
    (token, callbackMap) => {
      if (token.addr === zeroAddr) {
        utility.call(
          this.web3,
          this.contractTokenEther,
          this.config.contractTokenEtherAddr,
          'balanceOf',
...
TokenEther.prototype.transfer = function transfer(addr, inputAmount, toAddr) {
...
TokenEther.prototype.deposit = function deposit(addr, inputAmount) {
  let amount = utility.ethToWei(inputAmount, this.getDivisor(addr));
  const token = this.getToken(addr);
  if (amount <= 0) {
      ...
      (err, result) => {
        ...
        if (amount <= result) {
          utility.send(
            ...
            addr,
            'approve',
            [this.config.contractTokenEtherAddr, amount, { gas: this.config.gasApprove, value: 0 }],
            this.addrs[this.selectedAccount],
            this.pks[this.selectedAccount],
            this.nonce,
            (errSend, resultSend) => {
...
TokenEther.prototype.withdraw = function withdraw(addr, amountIn) {
...
TokenEther.prototype.order = function order(direction, amount, price, expires, refresh) {
...
TokenEther.prototype.publishOrder = function publishOrder(
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        hash, this.pks[this.selectedAccount], (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.config.contractTokenEtherAddr,
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
...
TokenEther.prototype.cancelOrder = function cancelOrder(orderIn) {
...
TokenEther.prototype.trade = function trade(kind, order, inputAmount) {
  if (this.addrs[this.selectedAccount].slice(0, 39) === '0x0000000000000000000000000000000000000') {
    this.alertError(
      "You haven't selected an account. Make sure you have an account selected from the Accounts dropdown in the upper right.");
    ga('send', {
...
TokenEther.prototype.blockTime = function blockTime(block) {
  return new Date(
    this.blockTimeSnapshot.date.getTime() +
      ((block - this.blockTimeSnapshot.blockNumber) * 1000 * this.secondsPerBlock));
};
TokenEther.prototype.addPending = function addPending(err, txsIn) {
  const txs = Array.isArray(txsIn) ? txsIn : [txsIn];
  txs.forEach((tx) => {
...
TokenEther.prototype.updateUrl = function updateUrl() {
...
TokenEther.prototype.getDivisor = function getDivisor(tokenOrAddress) {
...
TokenEther.prototype.getToken = function getToken(addrOrToken, name, decimals) {
  let result;
...
TokenEther.prototype.loadToken = function loadToken(addr, callback) {
...
TokenEther.prototype.selectToken = function selectToken(addrOrToken, name, decimals) {
...
TokenEther.prototype.selectBase = function selectBase(addrOrToken, name, decimals) {
...
TokenEther.prototype.selectTokenAndBase = function selectTokenAndBase(tokenAddr, baseAddr) {
...
TokenEther.prototype.displayBuySell = function displayBuySell(callback) {
...
TokenEther.prototype.displayTokenGuidesDropdown = function displayTokenGuidesDropdown() {
  const tokens = this.config.tokens.map(x => x);
  tokens.sort((a, b) => (a.name > b.name ? 1 : -1));
...
TokenEther.prototype.displayHelpDropdown = function displayHelpDropdown() {
  this.ejs(`${this.config.homeURL}/templates/helpDropdown.ejs`, 'helpDropdown', {});
};
TokenEther.prototype.displayHelp = function displayHelp(name) {
  $('#helpBody').html('');
...
TokenEther.prototype.displayScreencast = function displayScreencast(name) {
  $('#screencastBody').html('');
...
TokenEther.prototype.displayConnectionDescription = function displayConnectionDescription() {
  this.ejs(`${this.config.homeURL}/templates/connectionDescription.ejs`, 'connection', {
    connection: this.connection,
    contracts: this.config.contractTokenEtherAddrs,
    contractAddr: this.config.contractTokenEtherAddr,
    contractLink: `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${this.config.contractTokenEtherAddr}`,
  });
};
TokenEther.prototype.displayTokenGuide = function displayTokenGuide(name) {
...
TokenEther.prototype.checkContractUpgrade = function checkContractUpgrade() {
  if (
    (!this.selectedContract || this.selectedContract !== this.config.contractTokenEtherAddr) &&
    (this.addrs.length > 1 ||
...
TokenEther.prototype.loading = function loading(callback) {
  [
    'deposit',
    'withdraw',
    'buy',
...
TokenEther.prototype.refresh = function refresh(callback, forceEventRead, initMarket, token, base) {
  ...
  this.q.push((done) => {
    ...
    this.selectedContract = this.config.contractTokenEtherAddr;
...
TokenEther.prototype.refreshLoop = function refreshLoop() {
  const self = this;
  function loop() {
    self.refresh(() => {
...
TokenEther.prototype.initDisplays = function initDisplays(callback) {
  this.loading(() => {});
...
TokenEther.prototype.loadWeb3 = function loadWeb3(callback) {
  this.config = config;
  // web3
...
TokenEther.prototype.initContracts = function initContracts(callback) {
  this.web3.version.getNetwork((error, version) => {
    ...
    // connection
    this.config.contractTokenEtherAddr = this.config.contractTokenEtherAddrs[0].addr;
    // get accounts
    ...
    utility.loadContract(
      this.web3,
      this.config.contractTokenEther,
      this.config.contractTokenEtherAddr,
      (err, contractTokenEther) => {
        this.contractTokenEther = contractTokenEther;
        utility.loadContract(
          this.web3,
...
TokenEther.prototype.startTokenEther = function startTokenEther() {
...
const tokenEther = new TokenEther();

module.exports = { TokenEther: tokenEther, utility };
