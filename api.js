...
API.init = function init(callback, allContracts, path, provider, configName, lookbackIn) {
  ...
  this.web3.version.getNetwork((error, version) => {
    ...
    // path
    if (path) {
      this.config.contractTokenEther = path + this.config.contractTokenEther;
      this.config.contractToken = path + this.config.contractToken;
    }

    // contracts
    this.contractTokenEther = undefined;
    this.contractTokenEtherAddrs = [this.config.contractTokenEtherAddrs[0].addr];
    if (allContracts) {
      this.contractTokenEtherAddrs = this.config.contractTokenEtherAddrs.map(x => x.addr);
    }
    ...
    async.series(
      [
        (callbackSeries) => {
          this.utility.loadContract(
            ...
            this.contractTokenEtherAddrs[0],
            (err, contract) => {
              this.contractTokenEther = contract;
      ...
      () => {
        callback(null, {
          contractTokenEther: this.contractTokenEther,
...
API.logs = function logs(callback, lookbackIn) {
  this.utility.blockNumber(this.web3, (err, blockNumber) => {
    ...
    async.mapSeries(
      this.contractTokenEtherAddrs,
      (contractTokenEtherAddr, callbackMap) => {
        const blocks = Object.values(this.eventsCache)
          .filter(x => x.address === contractTokenEtherAddr)
          .map(x => x.blockNumber);
...
API.getTokenEtherBalance = function getTokenEtherBalance(addr, callback) {
  if (addr.length === 42) {
...
API.getTokenEtherTokenBalances = function getTokenEtherTokenBalances(addr, callback) {
  if (addr.length === 42) {
    async.reduce(
      this.config.tokens,
      {},
...
API.getUSDBalance = function getUSDBalance(addr, tokenPrices, callback) {
  async.parallel(
    [
      ...
      (callbackParallel) => {
        API.getTokenEtherTokenBalances(addr, callbackParallel);
      },
      (callbackParallel) => {
    ...
    (err, results) => {
      const balances = { Wallet: results[0], TokenEther: results[1] };
...
API.addOrderFromEvent = function addOrderFromEvent(event, callback) {
  if (event.event === 'Order' && event.address === this.contractTokenEtherAddrs[0]) {
...
API.getTrades = function getTrades(callback) {
  ...
  events.forEach((event) => {
    if (event.event === 'Trade' && this.contractTokenEtherAddrs.indexOf(event.address) >= 0) {
...
API.getDepositsWithdrawals = function getDepositsWithdrawals(callback) {
  ...
  const events = Object.values(this.eventsCache);
  events.forEach((event) => {
    if (event.event === 'Deposit' && this.contractTokenEtherAddrs.indexOf(event.address >= 0)) {
      ...
      event.event === 'Withdraw' && this.contractTokenEtherAddrs.indexOf(event.address) >= 0
    ) {
      if (event.args.amount.toNumber() > 0) {
        const token = API.getToken(event.args.token);
        depositsWithdrawals.push({
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        this.utility.sign(this.web3, addr, hash, pk, (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.contractTokenEtherAddrs[0],
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
