...
$(() => {
  $('body').on('click', '#accountSubmit', (e) => {
    ...
    bundle.TokenEther.addAccount($('#accountAddr').val(), $('#accountPk').val());
  });
});
...
function depositClick(addr) { // eslint-disable-line no-unused-vars
  bundle.TokenEther.deposit(addr, $(`#depositAmount${addr}`).val());
}
function withdrawClick(addr) { // eslint-disable-line no-unused-vars
  bundle.TokenEther.withdraw(addr, $(`#withdrawAmount${addr}`).val());
}
function transferClick(addr) { // eslint-disable-line no-unused-vars
  bundle.TokenEther.transfer(addr, $(`#transferAmount${addr}`).val(), $(`#transferTo${addr}`).val());
}
$(() => {});
