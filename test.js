  ...
  describe('Contract scenario', () => {
    ...
    it('Should place an order onchain, check availableVolume and amountFilled, then cancel', (done) => {
      function testCancel(expiresIn, orderNonce, tokenGet, tokenGive, amountGet, amountGive, amount, account1, callback) {
        ...
        web3.eth.getBlockNumber((err, blockNumber) => {
          if (err) callback(err);
          expires += blockNumber;
          utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'order', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, { gas: 1000000, value: 0 }], account1, undefined, 0, (err2) => {
            assert.equal(err2, undefined);
            utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'availableVolume', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, 0, '0x0', '0x0'], (err3, result3) => {
              assert.equal(result3.equals(amountGet), true);
              utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'amountFilled', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, 0, '0x0', '0x0'], (err4, result4) => {
                assert.equal(result4.equals(new BigNumber(0)), true);
                utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'cancelOrder', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, 0, '0x0', '0x0', { gas: 1000000, value: 0 }], account1, undefined, 0, (err5) => {
                  assert.equal(err5, undefined);
                  utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'availableVolume', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, 0, '0x0', '0x0'], (err6, result6) => {
                    assert.equal(result6.equals(new BigNumber(0)), true);
                    utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'amountFilled', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, 0, '0x0', '0x0'], (err7, result7) => {
      ...
      function testCancel(expiresIn, orderNonce, tokenGet, tokenGive, amountGet, amountGive, amount, account1, callback) {
        ...
        web3.eth.getBlockNumber((err, blockNumber) => {
          ...
          utility.sign(web3, account1, hash, undefined, (err2, sig) => {
            utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'availableVolume', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, sig.v, sig.r, sig.s], (err3, result3) => {
              assert.equal(result3.equals(amountGet), true);
              utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'amountFilled', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, sig.v, sig.r, sig.s], (err4, result4) => {
                assert.equal(result4.equals(new BigNumber(0)), true);
                utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'cancelOrder', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, sig.v, sig.r, sig.s, { gas: 1000000, value: 0 }], account1, undefined, 0, (err5) => {
                  assert.equal(err5, undefined);
                  utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'availableVolume', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, sig.v, sig.r, sig.s], (err6, result6) => {
                    assert.equal(result6.equals(new BigNumber(0)), true);
                    utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'amountFilled', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, sig.v, sig.r, sig.s], (err7, result7) => {
                      assert.equal(result7.equals(amountGet), true);
                      callback();
                    });
                  });
    ...
    it('Should do a trade and check available volume depletion', (done) => {
      web3.eth.getBlockNumber((err, blockNumber) => {
        ...
        const condensed = utility.pack([contractTokenEtherAddr, tokenGet, amountGet.toNumber(), tokenGive, amountGive.toNumber(), expires, orderNonce], [160, 160, 256, 160, 256, 256, 256]);
        const hash = sha256(new Buffer(condensed, 'hex'));
        const amount = amountGet.div(new BigNumber(2));
        utility.sign(web3, user, hash, undefined, (err2, sig) => {
          utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'trade', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, user, sig.v, sig.r, sig.s, amount, { gas: 1000000, value: 0 }], accounts[2], undefined, 0, (err3) => {
            assert.equal(err3, undefined);
            utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'availableVolume', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, user, sig.v, sig.r, sig.s], (err4, result4) => {
    ...
    it('Should do a self trade and check available volume depletion', (done) => {
      web3.eth.getBlockNumber((err, blockNumber) => {
        ...
        const amount = amountGet.div(new BigNumber(2));
        utility.sign(web3, user, hash, undefined, (err2, sig) => {
          utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'trade', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, user, sig.v, sig.r, sig.s, amount, { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err3) => {
            assert.equal(err3, undefined);
            utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'availableVolume', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, user, sig.v, sig.r, sig.s], (err4, result4) => {
              assert.equal(result4.equals(amountGet.minus(amount)), true);
              done();
    ...
    it('Should attempt some trades initiated onchain that should fail', (done) => {
        ...
        web3.eth.getBlockNumber((err, blockNumber) => {
          ...
          utility.testSend(web3, contractAccountLevels, contractAccountLevelsAddr, 'setAccountLevel', [account1, accountLevel, { gas: 1000000, value: 0 }], account1, undefined, 0, (err2) => {
            ...
            utility.testCall(web3, contractAccountLevels, contractAccountLevelsAddr, 'accountLevel', [account1], (err3) => {
              assert.equal(err3, undefined);
              utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'order', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, { gas: 1000000, value: 0 }], account1, undefined, 0, (err4) => {
                assert.equal(err4, undefined);
                utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'trade', [tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce, account1, 0, '0x0', '0x0', amount, { gas: 1000000, value: 0 }], account2, undefined, 0, (err5) => {
      ...
      utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'balanceOf', [contractToken1Addr, account1], () => {
        utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'balanceOf', [contractToken1Addr, account2], (err2, initialBalance12) => {
          utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'balanceOf', [contractToken2Addr, account1], (err3, initialBalance21) => {
            utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'balanceOf', [contractToken2Addr, account2], () => {
              const trades = [
    ...
    it('Should do a token withdrawal', (done) => {
      ...
      utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'balanceOf', [contractToken1Addr, accounts[1]], (err, initialBalance) => {
        utility.testCall(web3, contractToken1, contractToken1Addr, 'balanceOf', [accounts[1]], (err2, initialTokenBalance) => {
          utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'withdrawToken', [contractToken1Addr, amount, { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err3) => {
            assert.equal(err3, undefined);
            utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'balanceOf', [contractToken1Addr, accounts[1]], (err4, finalBalance) => {
    ...
    it('Should do an Ether withdrawal', (done) => {
      const amount = new BigNumber(utility.ethToWei(100));
      utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'balanceOf', [0, accounts[1]], (err, initialBalance) => {
        web3.eth.getBalance(contractTokenEtherAddr, (err2, initialEtherBalance) => {
          utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'withdraw', [amount, { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err3) => {
            assert.equal(err3, undefined);
            utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'balanceOf', [0, accounts[1]], (err4, finalBalance) => {
              web3.eth.getBalance(contractTokenEtherAddr, (err5, finalEtherBalance) => {
                assert.equal(finalBalance.equals(initialBalance.sub(amount)), true);
    ...
    it('Should change the account levels address and fail', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeAccountLevelsAddr', ['0x0', { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the account levels address and succeed', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeAccountLevelsAddr', ['0x0', { gas: 1000000, value: 0 }], admin, undefined, 0, (err) => {
        assert.equal(err, undefined);
        utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'accountLevelsAddr', [], (err2, result2) => {
    ...
    it('Should change the fee account and fail', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeAccount', ['0x0', { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the fee account and succeed', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeAccount', [accounts[1], { gas: 1000000, value: 0 }], admin, undefined, 0, (err) => {
        assert.equal(err, undefined);
        utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'feeAccount', [], (err2, result2) => {
    ...
    it('Should change the make fee and fail', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeMake', [feeMake, { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the make fee and fail because the make fee can only decrease', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeMake', [feeMake.mul(2), { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the make fee and succeed', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeMake', [feeMake.div(2), { gas: 1000000, value: 0 }], admin, undefined, 0, (err) => {
        assert.equal(err, undefined);
        utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'feeMake', [], (err2, result2) => {
    ...
    it('Should change the take fee and fail', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeTake', [feeTake, { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the take fee and fail because the take fee can only decrease', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeTake', [feeTake.mul(2), { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the take fee and fail because the take fee must exceed the rebate fee', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeTake', [feeTake.minus(new BigNumber(1)), { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the take fee and succeed', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeTake', [feeRebate.plus(new BigNumber(2)), { gas: 1000000, value: 0 }], admin, undefined, 0, (err) => {
        assert.equal(err, undefined);
        utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'feeTake', [], (err2, result2) => {
    ...
    it('Should change the rebate fee and fail', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeRebate', [feeRebate, { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the rebate fee and fail because the rebate fee can only increase', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeRebate', [feeRebate.div(2), { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the rebate fee and fail because the rebate fee must not exceed the take fee', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeRebate', [feeTake.plus(new BigNumber(1)), { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the rebate fee and succeed', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeFeeRebate', [feeTake.minus(new BigNumber(1)), { gas: 1000000, value: 0 }], admin, undefined, 0, (err) => {
        assert.equal(err, undefined);
        utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'feeRebate', [], (err2, result2) => {
    ...
    it('Should change the admin account and fail', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeAdmin', [accounts[1], { gas: 1000000, value: 0 }], accounts[1], undefined, 0, (err) => {
        assert.equal(!err, false);
        done();
      });
    });
    it('Should change the admin account and succeed', (done) => {
      utility.testSend(web3, contractTokenEther, contractTokenEtherAddr, 'changeAdmin', [accounts[1], { gas: 1000000, value: 0 }], admin, undefined, 0, (err) => {
        assert.equal(err, undefined);
        utility.testCall(web3, contractTokenEther, contractTokenEtherAddr, 'admin', [], (err2, result2) => {
          assert.equal(result2 === accounts[1], true);
          admin = result2;
          done();
        });
